...
def unpackjs(texto):
    ...
    if len(matches)>0:
        data = matches[0]
        plugintools.log("[unpackerjs.py] bloque funcion="+data)
    else:
        return ""
    ...
    scrapertools.printMatches(matches)
    cifrado = matches[0][0]
    plugintools.log("[unpackerjs.py] cifrado="+cifrado)
    plugintools.log("[unpackerjs.py] palabras="+matches[0][1])
    descifrado = ""
    ...
    for palabra in palabras:
        ...
        plugintools.log(claves[i]+"="+palabra)
        i=i+1
    ...
    def lookup(match):
        try:
            ...
            plugintools.log("[unpackerjs.py] Error al encontrar la clave "+match.group(0))
            return ""
    ...
    plugintools.log("descifrado="+descifrado);
